...
export const Restaurants = () => {
    ...
    return <div>
        ...
        <form onSubmit={handleSubmit(onSearchSubmit)} className="bg-gray-800 w-full py-40 flex items-center justify-center">
            <input type="Search" {...register("searchTerm", { required: true, min: 3 })} name="searchTerm" className="rounded-md input w-3/12 border-0" placeholder="Search Restaurants..." />
        </form>
        {!loading && (
                ...
                <div className="flex justify-around max-w-xs mx-auto">
                    {data?.allCategories.categories?.map((category) => (
                        ...
                        <Link to={`/category/${category.slug}`}>
                            <div key={category.id} className="cursor-pointer flex flex-col group items-center">
